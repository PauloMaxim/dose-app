...
export const readMyProfile = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await getSupabaseUserClient(context.accessToken)
      .from("profiles")
      .select("id, display_name, avatar_path, locale, timezone, onboarding_completed_at, created_at, updated_at")
...
export const replaceMyInterests = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: input, context }) => {
    ...
    const topicIds = input.interests.flatMap((item) =>
      "topicId" in item ? [item.topicId] : [],
    );
    const { error } = await getSupabaseUserClient(context.accessToken).rpc(
      "replace_my_interests",
      { specialty_ids: specialtyIds, topic_ids: topicIds },
    );
...
export const readMyNotificationPreferences = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await getSupabaseUserClient(context.accessToken)
      ...
      .select("email_enabled, push_enabled, digest_frequency, quiet_hours_start, quiet_hours_end, updated_at")
      .eq("user_id", context.userId)
      .single();
