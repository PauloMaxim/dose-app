...
function Login() {
  ...
  if (isPending || (user && (remoteOnboarding === "idle" || remoteOnboarding === "loading"))) {
    return (
  ...
  if (user) {
    return <Navigate to={remoteOnboarding === "complete" ? "/" : "/onboarding"} replace />;
  }
  ...
  return (
    <main className="flex h-full min-h-0 flex-col overflow-y-auto bg-bg px-6 py-8">
      ...
      {mode === "in" && (
        <button
          type="button"
          className="h-10 text-sm text-subtle"
          onClick={() => {
            setMode("forgot");
            setError("");
          }}
        ...
        <div className="mt-6 space-y-2">
          ...
          {GROK_PROVIDERS.map((p) => (
            <button
              ...
              className={cn("h-12 w-full rounded-full bg-card text-sm font-medium text-fg")}
            >
      ...
      <Link to="/onboarding" className="mt-6 mb-4 text-center text-sm text-subtle">
        Voltar ao início
      </Link>
    </main>
  );
